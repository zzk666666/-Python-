...
if __name__ == '__main__':
    tasks = []
    q=Manager().Queue()
    init=["0岁你出生了是个男孩","1岁你自学会了走路","2岁你学会了数独","3岁上了城里的幼儿园","4岁你学完了Python","5岁你学会C++、Java。","6岁你上了城里的普通小学","7岁你为班级同学写了表白程序，你父母没说你早恋。",
    ...
    younth=["塔利班组织闪击波兰,法国宣布投降。","日本武力攻占珍珠港","在云南发现太岁肉芝价值1兆","探险队在昆仑山发现九尾狐还会发光","中国经济超过美国","教育局宣布提倡中学生早恋","上海小学生春游登顶珠峰","你非常热爱你的工作，老板也觉得你善良。","上海新闻频道报道上海杨浦小学生神秘失踪事件，被发现已经像被妖怪吸走精气。","美国发现超人","乌克兰占领莫斯科","苏联再次合体","日本再次占领中国","日本占领加州","中国已是全球第一经济体",
        "江苏省省会改名苏州","解放军武力占领东京","台湾回归，一国三制开始实行。"]
    Middle_aged=["你儿子在班里被女同学欺负，你儿子反击抓了她的敏感部位，被叫去德育处。","朝鲜统一韩国","新闻联播在四川某地发现神秘生物疑似女鬼","你成为本世纪科技人物","你做噩梦梦见中学时老师打你","小学课本已经出现物理","啥也没发生","中国改国号中华帝国被国际承认","塔利班占领华盛顿","埃塞俄比亚经济直逼中国","上海超群大厦竣工高度8000米超越珠穆朗玛峰","你女儿吃烧烤遇到猥琐男，女儿用空手道击退对方，并一打10。","南京发现日本兵的鬼魂，被人群殴。","港澳台允许服兵役"
    ...
    death=["重重的摔了一跤","你被车撞死了","你在国外被枪击死"]
    deathofbaby=["你胎死腹中","你被父母故意摔在地上","你妈因为早孕你外公外婆叫你妈把你被打掉了","你妈不想要你直接打胎","你妈堕胎了"]
    after1=[]
    after2=[]
    after3=[]
    length1=len(younth)
    ...
    for i in init:
        ...
        if sum==r:
            if r==0:
                print(random.choice(deathofbaby))
                break
            else:
                print(f"{sum1}岁 {random.choice(death)}")
                break
        else:
            print(q.get(i))
            time.sleep(1)
    ...
    for i in l2:
        ...
        if sum2 == r2:
            print(f"{sum2}岁 {random.choice(death)}")
            break
        else:
            print(f"{sum2}岁 {i}")
            time.sleep(1)
